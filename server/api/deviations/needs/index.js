...
export default defineEventHandler(async (event) => {
    ...
    if (method === 'get') {
        return handleDatabaseOperation('DeviationNeeds', 'get');
    } else if (method === 'post') {
        const postData = await readBody(event);
        const existingData = await db.ref(`DeviationNeeds/${postData.id}`).once('value');
        ...
        return handleDatabaseOperation(`DeviationNeeds/${postData.id}`, 'post', postData);
    } else {
