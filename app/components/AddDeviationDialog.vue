					...
					<div class="flex flex-col gap-2">
						<div class="flex flex-row gap-2">
							...
							<div class="flex flex-col gap-2">
								...
								<select id="dev-id"
									class="select w-full md:w-[110px] bg-white !outline-none text-gray-500/50"
									v-model="devID">
									<option disabled selected value="">Pick ID</option>
									<option v-for="(id, index) in getAvailableIds(deviations)"
										:key="'dev-id-option-' + index" :value="id">
										{{ id }}
									</option>
								</select>
							</div>
